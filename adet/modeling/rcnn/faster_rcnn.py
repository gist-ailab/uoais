...
def fast_rcnn_inference(boxes, scores, occ_scores, image_shapes, score_thresh, nms_thresh, topk_per_image):
    ...
    result_per_image = [
        fast_rcnn_inference_single_image(
            boxes_per_image, scores_per_image, occ_scores_per_image, image_shape, score_thresh, nms_thresh, topk_per_image
        )
        for scores_per_image, occ_scores_per_image, boxes_per_image, image_shape in zip(scores, occ_scores, boxes, image_shapes)
    ]
    return tuple(list(x) for x in zip(*result_per_image))


def fast_rcnn_inference_single_image(
    boxes, scores, occ_scores, image_shape, score_thresh, nms_thresh, topk_per_image
):
    """
    ...
    # print(occ_scores.shape, filter_mask.shape)
    # occ_scores = occ_scores[filter_mask]
    # occ_preds = occ_preds[filter_mask]
...
class FastRCNNOutputs(object):
    ...
    def __init__(
        self, box2box_transform, pred_class_logits, pred_proposal_deltas, proposals, smooth_l1_beta, pred_occlusion_class_logits
    ):
        """
        Args:
    ...
    def occ_softmax_cross_entropy_loss(self):
        ...
        # print("pred:", torch.max(F.softmax(self.pred_occlusion_class_logits, dim=-1), 1)[1], torch.sum(torch.max(F.softmax(self.pred_occlusion_class_logits, dim=-1), 1)[1]))
        # print("gt:", self.gt_occludeds, torch.sum(self.gt_occludeds))
        # print("========")
        n_occ = torch.sum(self.gt_occludeds)
        n_gt = self.gt_occludeds.shape[0]
        n_noocc = n_gt - n_occ
        # return F.cross_entropy(self.pred_occlusion_class_logits, self.gt_occludeds, weight=torch.Tensor([1, 4]).to(device=self.gt_occludeds.device), reduction="mean")
        return F.cross_entropy(self.pred_occlusion_class_logits, self.gt_occludeds, weight=torch.Tensor([1, n_noocc/n_occ]).to(device=self.gt_occludeds.device), reduction="mean")
    ...
    def inference(self, score_thresh, nms_thresh, topk_per_image):
        ...
        return fast_rcnn_inference(
            boxes, scores, occ_scores, image_shapes, score_thresh, nms_thresh, topk_per_image
        )
