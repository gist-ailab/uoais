...
from .resnet_depth import build_depth_resnet_backbone
from .resnet_r2d_guide import build_rgbd_cbam_resnet_backbone

import fvcore.nn.weight_init as weight_init


__all__ = ["build_resnet_rgbd_earlyfusion_fpn_backbone", "build_resnet_rgbd_latefusion_fpn_backbone", "FPN"]
...
class FPN(Backbone):
    ...
    def __init__(
        self, bottom_up, in_features, out_channels, norm="", top_block=None, fuse_type="add"
    ):
        """
        Args:
            ...
            fuse_type (str): types for fusing the top down features and the lateral
                ones. It can be "add" (default), which adds up element-wise; or "avg",
...
def build_resnet_rgbd_earlyfusion_fpn_backbone(cfg, input_shape: ShapeSpec):
    """
    Args:
    ...
    bottom_up.conv1 = nn.Conv2d(4, 64, kernel_size=7, stride=2, padding=3, bias=False)
    weight_init.c2_msra_fill(bottom_up.conv1)
...
@BACKBONE_REGISTRY.register()
def build_fcos_resnet_rgbd_latefusion_fpn_backbone(cfg, input_shape: ShapeSpec):
    """
    Args:
        cfg: a detectron2 CfgNode
...
@BACKBONE_REGISTRY.register()
def build_resnet_rgbd_latefusion_cbam_guide_fpn_backbone(cfg, input_shape: ShapeSpec):
    """
    Args:
        cfg: a detectron2 CfgNode
    Returns:
    ...
    """
    input_shape = ShapeSpec(channels=3)
    bottom_up = build_rgbd_cbam_resnet_backbone(cfg, input_shape)
    in_features = cfg.MODEL.FPN.IN_FEATURES
    out_channels = cfg.MODEL.FPN.OUT_CHANNELS
    backbone = RGBD_FPN(
        bottom_up = bottom_up,
